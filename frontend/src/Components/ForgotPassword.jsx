...
const ForgotPassword = () => {
  ...
  const handlePasswordChange = async (values) => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/forget/password-change/${role}`,
        values,
        {
          headers: {
  ...
  const handleValidate = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/forget/validate`,
        { otp: otpinput },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("UserForgetToken")}`,
      ...
      <div className="px-4 py-5 border-2 border-gray-300 rounded-lg shadow-lg w-[350px]">
        {!otpvalidate && (
          <div className="flex items-center flex-col gap-2">
            ...
            <button
              className="bg-green-500 text-white px-4 py-2 text-[20px] rounded-full"
        ...
        {otpvalidate && (
          <form onSubmit={formik.handleSubmit}>
            <div>
              <label htmlFor="newPassword" className="text-[18px] block mb-1">
              ...
              <label
                ...
                className="text-[18px] block mt-3 mb-1"
              >
            ...
            <div className="w-full flex mt-4">
              <button
                type="submit"
                className="mx-auto bg-green-200 px-4 py-2 rounded-xl text-lg"
              >
                Submit
              </button>
            </div>
