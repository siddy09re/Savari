...
const MapBoxContainer = ({longtitude , latitude , mapboxgl, mapRef , pickupcoord , destinationcoord , setdistance , drivercoord , TravelDetails }) => {
...
if (destinationMarkerRef.current) {
  ...
  useEffect(() => {
    ...
    if(!pickupcoord || !drivercoord || !mapRef.current) {
      // Remove route layer if it exists
    ...
    const getDriverRoute = async() => {
      ...
      const geojson = {
        ...
        if (data ) {
          ...
          if(TravelDetails){
            socket.emit('DriverDistance' , {DriverDistance : DriverDistance , TravelDetails : TravelDetails});
...
if (pickupMarkerRef.current) {
  ...
  },[drivercoord,mapRef,mapboxgl,pickupcoord ,socket,dispatch])
