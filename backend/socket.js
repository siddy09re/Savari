...
function initializeSocket(server) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('CancelRide', async (rideId) => {
          ...
          socket.on('RideIgnored' , async(data) => {
            const TravelDetails = data.TravelDetails;
            const rideId = TravelDetails._id;

            const NewRide = await ridemodel.findByIdAndUpdate(rideId,{status : "pending"} , {new : true});
            console.log("The new ride is ", NewRide);

            const UserSocketId = await userModel.findById(TravelDetails.user).select('socketId');
            const socketId = UserSocketId?.socketId;
            
            sendMessageToSocketId(socketId, {
              event: "RideIgnoredUser",
              data: { success: true }
            });
