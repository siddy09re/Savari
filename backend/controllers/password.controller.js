...
module.exports.passwordchange = async (req,res,next) => {
      ...
      const hashedPassword = await bcrypt.hash(values.newPassword , 10);
      const Passwordchange = await captainmodel.findByIdAndUpdate(captain._id, {password : hashedPassword} , {new : true});
      if(!Passwordchange){
          return res.status(500).json({message : "Error in changing password"}); 
      }
      return res.status(200).json({message : "Captain password change endpoint hit successfully", values : values});
    }   
}
